 ...
 * @copyright CiviCRM LLC (c) 2004-2019
 */
class CRM_Contributiondetailmod_Form_Report_ContributionDetailMod extends CRM_Report_Form {
  ...
  GROUP BY {$this->_aliases['civicrm_contribution']}.currency";
    ...
    public function beginPostProcessCommon() {
      ...
      $sql = "UPDATE civireport_contribution_detail_temp2 SET civicrm_contribution_contribution_or_soft = 'Soft Credit';";
  ...
  (SELECT * FROM civireport_contribution_detail_temp2)");
      ...
      $sql = "SELECT civicrm_contribution_contribution_id as contribution_id, count(*) as count FROM civireport_contribution_detail_temp3 GROUP BY contribution_id HAVING count(*) > 1";
      $result = CRM_Core_DAO::executeQuery($sql);
      while ($result->fetch()) {
        $sql = "UPDATE civireport_contribution_detail_temp3 SET civicrm_contribution_total_amount_sum = civicrm_contribution_total_amount_sum / {$result->count} WHERE civicrm_contribution_contribution_id = {$result->contribution_id} AND civicrm_contribution_contribution_or_soft = 'Contribution'";
        $result = CRM_Core_DAO::executeQuery($sql);
      }
      if ($this->isContributionBaseMode) {
        $sql = "DELETE FROM civireport_contribution_detail_temp3 WHERE civicrm_contribution_contribution_or_soft = 'Soft Credit'";
        $result = CRM_Core_DAO::executeQuery($sql);
      }
